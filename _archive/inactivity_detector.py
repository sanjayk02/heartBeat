...
class InactivityDetector:
    """
    InactivityDetector class monitors user activity based on keyboard, mouse, and screen changes.
    Attributes:
        total_runtime (int): Total runtime for the detector in seconds.
        timeout (int): Time in seconds to consider the user inactive.
        check_interval (int): Interval in seconds to check for user activity.
        region (tuple): Region of the screen to monitor for changes (left, top, width, height).
        last_activity_time (float): Timestamp of the last detected activity.
        active (bool): Indicates if the user is currently active.
        start_time (float): Timestamp when the monitoring started.
        active_time (float): Total time the user has been active.
        inactive_time (float): Total time the user has been inactive.
        last_check_time (float): Timestamp of the last activity check.
        last_screenshot (Image): The last captured screenshot.
        icon (pystray.Icon): System tray icon for the detector.
        running (bool): Indicates if the detector is running.
        mouse_listener (mouse.Listener): Listener for mouse events.
        keyboard_listener (keyboard.Listener): Listener for keyboard events.
    Methods:
        __init__(self, total_runtime=3600, timeout=5, check_interval=2, region=(100, 100, 500, 400)):
            Initializes the InactivityDetector with the given parameters.
        on_activity(self, *args):
            Resets the inactivity timer on user interaction.
        take_screenshot(self):
            Captures a portion of the screen.
        compare_screenshots(self, img1, img2):
        ...
        create_icon_image(self):
            Creates a simple icon image.
        update_tray_time(self):
